...
const KYC = () => {
  ...
  const { completeKyc, user, checkKycStatus, kycVerified } = useAuth();
  ...
  // Check KYC status from backend on component mount
  useEffect(() => {
    const verifyKycStatus = async () => {
      ...
      try {
        ...
        if (result.verified) {
          // User is already KYC verified, redirect to dashboard
          console.log('KYC already verified, redirecting to dashboard');
      ...
      try {
        ...
        if (isMatch) {
          try {
            ...
            // Call backend to update KYC status
            const token = await user.getIdToken();
            const res = await fetch(
              "https://commitchaosbackend.onrender.com/kyc",
              {
                method: "POST",
                headers: {
            ...
            console.log("Backend KYC update successful");
          } catch (backendErr) {
            console.error("Backend KYC update failed", backendErr);
            // Optional: decide if you want to fail the whole process or just log it
            // For now proceeding as the face match was valid
          }
  ...
  // Show loading state while checking KYC status from backend
  if (checkingKycStatus) {
    return (
      <div className="min-h-screen bg-global-bg flex items-center justify-center">
        <div className="text-center">
          <div className="w-16 h-16 mx-auto mb-4 border-4 border-global-indigo border-t-transparent rounded-full animate-spin" />
          <p className="text-global-muted">Checking verification status...</p>
        </div>
      </div>
    );
  }
